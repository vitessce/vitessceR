...
When working with the `vitessce` R package, you may encounter issues, particularly related to file formatting, file permissions, URL typos, etc. Below, we list some debugging methods that may help to resolve these bugs.

If none of these tips help, please write an issue on [GitHub](https://github.com/vitessce/vitessce-r/issues).

The following code snippets assume that `vitessce` has been loaded via `library(vitessce)` and the Vitessce configuration instance is stored in the variable `vc`:
